export class CrunchItemSheet extends foundry.applications.api.DocumentSheetV2 {
  constructor(...args) {
    super(...args);
    console.log("🎯 CrunchItemSheet constructed", this);
   ...
   */
  get template() {
    return `systems/crunch/templates/item-sheet.html`;
  }
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: "crunch-item-sheet",
      classes: ["crunch", "sheet", "item"],
      title: "Crunch Item Sheet", 
      width: 500, 
      height: "auto", 
      resizable: true,
  ...
  /**
   * Getter for the Item document (this.object).
   */
  get item() {
    return this.object;
  }
  ...
  async getData(options={}) {
    ...
    context.debug = {
      ...
      hasSystem: !!this.item.system, 
      templatePath: this.template,
      renderTime: Date.now()
    };
    
    console.log("🔍 Final item sheet context (from getData):", context); 
    return context;
  }
  ...
  async _renderHTML(context, options) {
    console.log("🎨 _renderHTML: Rendering template for item context:", context);
  ...
  async _replaceHTML(result, content, options) {
    console.log("🔄 _replaceHTML: Replacing Item HTML content.");
  ...
  activateListeners(html) {
    ...
    // Listener for timeCost input
    html.find('input[name="system.timeCost"]').on('change', event => {
      const value = parseInt(event.currentTarget.value) || 0;
      this.item.update({ "system.timeCost": value });
    });

    // Place any other custom event listeners here.
  }
}